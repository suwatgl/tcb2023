...
export default {
  ...
  methods: {
    ...
    showForm(patient_id, patient) {
      ...
      if (patient_id) {
        this.axios.get(`tcb/patients?t=get-patient-by-id&id=${patient_id}`).then((res1) => {
          this.curr_patient = res1.data
        })
        this.axios.get(`tcb/cancersums?t=get-default-new-cancer&patient_id=${patient_id}`).then((response) => {
          this.curr_cancer = response.data
          this.curr_cancer.patient_id = patient_id
          this.curr_cancer.entrance_date = null
    ...
    handleSubmit() {
      this.$refs.form.validate().then((success) => {
        ...
        let loader = this.$loading.show()
        this.axios
          .post('tcb/cancers', param, { headers: { 'Content-Type': 'multipart/form-data' } })
          .then((res) => {
            if (res.data.status) {
              // case ที่ไม่ต้อง update ใบสรุป
              if ([1, 4, 5].includes(res.data.type_id)) {
                // 1 สร้างใบสรุปใหม่ เนื่องจาก topo ไม่ตรงกับ ใบสรุปล่าสุด (Multiple Primary)
                // 4 สร้างใบสรุปใหม่ เนื่องจาก ไม่พบใบสรุปที่ตรงกับ topo ปัจจุบัน
                // 5 กรณีผู้ใช้เป็น Anywhere (ไม่ได้เป็น "ทะเบียนมะเร็ง") ไม่ต้องไปหน้า Create/Update ใบสรุป
                this.$nextTick(() => {
                ...
                // Update ใบสรุปล่าสุด
                // 2, 3 บันทึก Cancer ล่าสุด และ นำข้อมูลโรคมะเร็งไป Update ใบสรุปใหม่ ในหน้า 3 คอลลัม
                let new_cancer_sum = res.data.cancer_receive.cancerData
                localStorage.setItem('new_cancer_sum', JSON.stringify(new_cancer_sum))
                this.$nextTick(() => {
                  this.$bvModal.hide('modal-patient-form-cancer')
                  loader.hide()
                  // ถ้าเป็น ทะเบียนมะเร็ง ให้ไปทำใบสรุป ต่อ
                  if (this.currentUser.roleCancer == 'Y') {
              ...
              if (res.data.confirm) {
                this.$bvModal
                  .msgBoxConfirm(res.data.message, {
                    title: 'กรุณายืนยัน',
                    size: 'sm',
                    buttonSize: 'sm',
                    okVariant: 'danger',
