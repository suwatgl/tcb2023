<template>
  ...
  <div class="menu" v-if="menus">
    ...
    <div v-for="menu in menus" v-bind:key="menu.path">
      <sidebar-nav-list ref="sidebarNavList" v-bind:menu="menu" v-bind:scrollTop="scrollTop" v-bind:status="menu.status" v-on:collapse-other="handleCollapseOther(menu)" v-on:show-float-submenu="handleShowFloatSubmenu" v-on:hide-float-submenu="handleHideFloatSubmenu"></sidebar-nav-list>
    </div>
...
export default {
  ...
  computed: {
    ...
    menus() {
      function getMenus(menus, userRoles) {
        let newMenus = []
        menus.forEach(function (menu) {
          if (menu.auth) {
            if (menu.roles) {
              let diff = userRoles.filter((x) => menu.roles.includes(x))
              if (diff.length > 0) {
                if (menu.children) {
                  menu.children = getMenus(menu.children, userRoles)
                }
                newMenus.push(menu)
              }
            } else {
      ...
      return getMenus(this.listMenus, this.currentUser ? this.currentUser.roles : [])
    },
  },
  methods: {
